...
public class AtomicAttraction : MonoBehaviour {
	...
	void AnimatePosition()
	{
		if (_animatePos) {
			if (_posAnimBuffered) {
				_animTimer += Time.deltaTime * AudioPeer._audioBandBuffer[_posAnimBand] * _posAnimSpeed;
			} else {
				_animTimer += Time.deltaTime * AudioPeer._audioBand[_posAnimBand] * _posAnimSpeed;
			}
			if (_animTimer >= 1) {
				_animTimer -= 1f;
			}
